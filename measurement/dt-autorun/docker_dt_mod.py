...
class ExpRunner:
    ...
    def move_log(self):
        ...
        def _moving(src, dst, files):
            ...
            dst_ = expanduser(dst)
            for _f in files:
                shutil.copy2(join(src_, _f), join(dst_, _f))
        ...
        _moving("~/autorun/logs/cpu", dst_folder, n_cpu - e_cpu)
        _moving("~/autorun/logs/net", dst_folder, n_net - e_net)
        _moving("~/autorun/horovod_logs/hooks", dst_folder, n_hook-e_hook)
        _moving("~/autorun/horovod_logs/model_log/", dst_folder, n_model-e_model)
        _moving("~/autorun/horovod_logs/mpi_events", dst_folder, n_mpi-e_mpi)
